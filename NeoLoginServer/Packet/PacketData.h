...
public:
	...
	int32_t GetSize(){
		size_t size =0;
		size += sizeof(int16_t) + (sizeof(wchar_t) * id.length()) + sizeof(int16_t) + (sizeof(wchar_t) * name.length()) + sizeof(int16_t) + (sizeof(wchar_t) * email.length()) + sizeof(int16_t) + (sizeof(wchar_t) * password.length()) + sizeof(int16_t) + (sizeof(wchar_t) * passwordCheck.length()) ;
	...
	void Deserialize(InputMemoryStream& buffer){
		...
		buffer.Read(passwordCheck);

	}
};

class P_C_REQ_CREATE_ID_CHECK
 : public Packet{
public:
	PacketID GetID(){
